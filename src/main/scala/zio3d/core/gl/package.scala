...
import org.lwjgl.opengl.{GL11, GL13, GL15, GL20, GL30, GLCapabilities}
...
package object gl {
  ...
  type GL = Has[GL.Service]

  object GL extends Serializable {
    ...
    val live = ZLayer.succeed {
      new Service {
        ...
        def deleteShader(shader: Shader): UIO[Unit] =
          IO.effectTotal {
            GL20.glDeleteShader(shader.ref)
          }
        ...
        def useProgram(program: Program): UIO[Unit] =
          IO.effectTotal {
            GL20.glUseProgram(program.ref)
          }

        def deleteProgram(program: Program): UIO[Unit] =
          IO.effectTotal {
            GL20.glDeleteProgram(program.ref)
          }

        def shaderSource(shader: Shader, src: CharSequence*): UIO[Unit] =
          IO.effectTotal {
            GL20.glShaderSource(shader.ref, src: _*)
          }

        def compileShader(shader: Shader): UIO[Unit] =
          IO.effectTotal {
            GL20.glCompileShader(shader.ref)
          }

        def getShaderi(shader: Shader, pname: Int): UIO[Int] =
          IO.effectTotal {
            GL20.glGetShaderi(shader.ref, pname)
          }

        def getShaderInfoLog(shader: Shader): UIO[String] =
          IO.effectTotal {
            GL20.glGetShaderInfoLog(shader.ref)
          }

        def attachShader(program: Program, shader: Shader): UIO[Unit] =
          IO.effectTotal {
            GL20.glAttachShader(program.ref, shader.ref)
          }

        def linkProgram(program: Program): UIO[Unit] =
          IO.effectTotal {
            GL20.glLinkProgram(program.ref)
          }

        def getProgrami(program: Program, pname: Int): UIO[Int] =
          IO.effectTotal {
            GL20.glGetProgrami(program.ref, pname)
          }

        def getProgramInfoLog(program: Program): UIO[String] =
          IO.effectTotal {
            GL20.glGetProgramInfoLog(program.ref)
          }
        ...
        def bindBuffer(target: Int, buffer: VertexBufferObject): UIO[Unit] =
          IO.effectTotal {
            GL15.glBindBuffer(target, buffer.ref)
          }

        def deleteBuffers(buffer: VertexBufferObject): UIO[Unit] =
          IO.effectTotal {
            GL15.glDeleteBuffers(buffer.ref)
          }
        ...
        def getAttribLocation(program: Program, name: String): UIO[AttribLocation] =
          IO.effectTotal {
            AttribLocation(GL20.glGetAttribLocation(program.ref, name))
          }

        def enableVertexAttribArray(index: AttribLocation): UIO[Unit] =
          IO.effectTotal {
            GL20.glEnableVertexAttribArray(index.loc)
          }
        ...
        def vertexAttribPointer(
          ...
          IO.effectTotal {
            GL20.glVertexAttribPointer(index.loc, size, `type`, normalized, stride, pointer)
          }
        ...
        def disableVertexAttribArray(index: AttribLocation): UIO[Unit] =
          IO.effectTotal {
            GL20.glDisableVertexAttribArray(index.loc)
          }
        ...
        def bindVertexArray(array: VertexArrayObject): UIO[Unit] =
          IO.effectTotal {
            GL30.glBindVertexArray(array.ref)
          }

        def deleteVertexArrays(array: VertexArrayObject): UIO[Unit] =
          IO.effectTotal {
            GL30.glDeleteVertexArrays(array.ref)
          }

        def bindFragDataLocation(program: Program, colorNumber: Int, name: String): UIO[Unit] =
          IO.effectTotal {
            GL30.glBindFragDataLocation(program.ref, colorNumber, name)
          }

        def uniformMatrix4fv(location: UniformLocation, transpose: Boolean, value: FloatBuffer): UIO[Unit] =
          IO.effectTotal {
            GL20.glUniformMatrix4fv(location.loc, transpose, value)
          }
        ...
        def uniform1i(uniform: UniformLocation, value: Int): UIO[Unit] =
          IO.effectTotal {
            GL20.glUniform1i(uniform.loc, value)
          }
        ...
        def uniform3f(uniform: UniformLocation, value1: Float, value2: Float, value3: Float): UIO[Unit] =
          IO.effectTotal {
            GL20.glUniform3f(uniform.loc, value1, value2, value3)
          }
        ...
        def uniform4f(uniform: UniformLocation, value1: Float, value2: Float, value3: Float, value4: Float): UIO[Unit] =
          IO.effectTotal {
            GL20.glUniform4f(uniform.loc, value1, value2, value3, value4)
          }
        ...
        def getUniformLocation(program: Program, name: String): UIO[UniformLocation] =
          IO.effectTotal {
            UniformLocation(GL20.glGetUniformLocation(program.ref, name))
          }
        ...
        def bindTexture(target: Int, texture: Texture): UIO[Unit] =
          IO.effectTotal {
            GL11.glBindTexture(target, texture.value)
          }
        ...
        def compileShader(shaderType: Int, src: String) =
          for {
            ...
            _ <- shaderSource(s, src)
            _ <- compileShader(s)
            ok <- getShaderi(s, GL20.GL_COMPILE_STATUS)
            _ <- getShaderInfoLog(s).flatMap(log => IO.fail(log)).when(ok == 0)
          } yield s
      }
    }
  }
