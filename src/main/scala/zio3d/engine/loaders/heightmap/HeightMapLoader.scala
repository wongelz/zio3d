package zio3d.engine.loaders.heightmap
...
trait HeightMapLoader {
  def heightMapLoader: HeightMapLoader.Service
}

object HeightMapLoader {
  trait Service {
    def load(
  ...
  trait Live extends SceneShaderInterpreter.Live with HeightMapLoader {
    val heightMapLoader = new Service {
      ...
      def getHeight(x: Int, z: Int, width: Int, minY: Float, maxY: Float, buffer: ByteBuffer) = {
        ...
        minY + Math.abs(maxY - minY) * (argb.toFloat / MaxColour)
      }
    }
  }
}
