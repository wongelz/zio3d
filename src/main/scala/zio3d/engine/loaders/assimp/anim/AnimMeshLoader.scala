...
trait AnimMeshLoader {
  def animMeshLoader: AnimMeshLoader.Service
}

object AnimMeshLoader {
  ...
  trait Live extends Assimp.Live with AnimMeshLoader {
    val animMeshLoader = new Service {
...
//          rootNode  <- processAnimations(s, bones)
      ...
      private def getTransformationMatrices(nodeAnim: AINodeAnim) =
        Stream
          .range(0, nodeAnim.mNumPositionKeys() - 1)
          .map { i =>
